...
contract EthereumBridge is TokenRegistry, Ownable {
    ...
    function validateAndExecuteProof(
        ...
        (bool status, string memory message) = HarmonyProver.verifyHeader(header, mmrProof);
        require(
            status,
            "block header could not be verified"
        );
