...
library Keccak512 {
    ...
    function sponge(uint[9] memory M) pure private returns(uint[16] memory) {
...
*/
   function sha3_512(bytes memory data) pure public returns(bytes memory) {
       ...
       uint[9] memory M;
       for(uint i = 0; i < dataWords; i++) {
           uint dataOffset = i * 8;
           M[i] = uint(uint8(data[dataOffset])) |
                  uint(uint8(data[dataOffset+1])) << 8 |
                  uint(uint8(data[dataOffset+2])) << 16 |
                  uint(uint8(data[dataOffset+3])) << 24 |
                  uint(uint8(data[dataOffset+4])) << 32 |
                  uint(uint8(data[dataOffset+5])) << 40 |
                  uint(uint8(data[dataOffset+6])) << 48 |
                  uint(uint8(data[dataOffset+7])) << 56;
       }
       M[dataWords] = 1;
       M[8] |= 0x8000000000000000;
       uint[16] memory result32 = sponge(M);
