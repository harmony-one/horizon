...
class HmyBridge {
    constructor(bridgeAddress, tokenAddress) {
        ...
        if (bridgeAddress) {
            ...
            console.log(" \n initialise HmyBridge.js with bridgeAddress: "+ this.bridgeAddress)
        } else {
        ...
        if (tokenAddress) {
            ...
            console.log(" \n initialise HmyBridge.js with hrc tokenAddress: ", this.tokenAddress)
        ...
        this.ethClientAddr = "0x8824f1a1c3b368ff6e8deaf5d0bfc9d760c416e6" //await deploy_contract(json) in elc/ethClient/test/deploy.js
        this.ethClientContract = hmy.contracts.createContract(
        clientJson.abi,
        this.ethClientAddr);

        console.log(" \n connecting to Harmony chain: "+ process.env.HMY_NODE_URL )
        console.log(" \n ELC contract address on Harmony chain: "+ this.ethClientAddr)
    ...
    async waitBlockHashSafeOnEthClient(txHash){
        ...
        console.log(" \n wait till block hash of transaction in block no: "+ txBlockNumber + " is recorded on eth client contract on hmy chain  ")
    ...
    async deployBridge() {
        ...
        console.log("updating eth client contract address "+ this.ethClientAddr + " in bridge contract ")
        
         response = await this.bridgeContract.methods.changeLightClient(this.ethClientAddr).send(options)
    ...
    async updateLightClient(){
        const options = {gasPrice: this.gasPrice, gasLimit: this.gasLimit}
        console.log("updating eth client contract address "+ this.ethClientAddr + " in bridge contract ")        
         let response = await this.bridgeContract.methods.changeLightClient(this.ethClientAddr).send(options)
