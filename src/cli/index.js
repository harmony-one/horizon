...
Dag_CMD.command('generate <epoch_start>')
    ...
    .action((start, options) => {
        start = Number(start)
        const dagMTrees = genearateDagMTreeRange(
            options.dagDir,
            start,
            options.num
        )
...
Dag_CMD.command('blockProof')
    ...
    .option(
        '-H --header <rlpBlockHeader>',
        'hexadecimal string of rlp block header'
    )
    .option(
        '-d,--dagDir <dag dir>',
        'direction to store dag merkel tree',
...
CMD_ELC.command('status <hmyUrl> <ELC_address>')
    .description('relay eth block header to elc on hmy')
    .action(async (hmyUrl, elcAddress) => {
        await statusELC(hmyUrl, elcAddress)
    })

const ETHRelay_CMD = program
    .command('ethRelay')
    .description('ethereum block relay cli')
ETHRelay_CMD.command('getBlockHeader <ethUrl> <number/hash>')
...
ETHRelay_CMD.command('relay <ethUrl> <hmyUrl> <elcAddress>')
    .description('relay eth block header to elc on hmy')
    .option(
        '-d,--dagDir <dag dir>',
        'direction to store dag merkel tree',
...
const CMD_EProver = program
    .command('EProver')
    .description('ethereum receipt prove cli')

CMD_EProver.command('proof <ethUrl> <tx_hash>')
...
const CMD_EVerifier = program
    .command('EVerifier')
    .description('ethereum receipt verify cli')

CMD_EVerifier.command('verify <ethUrl> <tx_hash> <hmyUrl> <contract address>')
    .description(
        'verify receipt MPT proof vai everifier contract, return receipt'
    )
    .option('-o,--output <OUTPUT>', 'output file')
    .option('-t --type <output format>', 'output format: json/rlp', 'json')
...
CMD_EVerifier.command('deploy <hmyUrl>')
...
CMD_Bridge.command('deploy <ethUrl> <hmyUrl>')
    .description(
        'deploy bridge contract on ethereum and harmony and then bind these'
    )
...
CMD_Bridge.command(
    ...
    .action(
        async (ethUrl, ethAddress, hmyUrl, hmyAddress, token, receipt, amount) => {
            const { ethBridge, hmyBridge } = await tokenTo(
                ethUrl,
                ethAddress,
                hmyUrl,
                hmyAddress,
...
CMD_Bridge.command(
    ...
    .action(
        async (hmyUrl, hmyAddress, ethUrl, ethAddress, token, receipt, amount) => {
            const { hmyBridge, ethBridge } = await tokenBack(
                hmyUrl,
                hmyAddress,
                ethUrl,
                ethAddress,
...
CMD_Bridge.command('deployFaucet <rpcUrl>')
...
CMD_Bridge.command('change <rpcUrl> <bridgeAddress> <ligthClient>')
...
CMD_Bridge.command('deployFakeClient <rpcUrl>')
