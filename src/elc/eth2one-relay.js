...
async function main () {
  const clientSol = ElcAddress
    ? await ClientSol.at(ElcAddress)
    : await ClientSol.deployed()
  ...
  a = await clientSol.addBlockHeader(rlpHeader, proofs.dagData, proofs.proofs, {
    gas: 5000000
  })
...
module.exports = function (result) {
  return main().then(result).catch(result)
}
