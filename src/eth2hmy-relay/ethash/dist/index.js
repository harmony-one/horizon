...
class Ethash {
  ...
  run(val, nonce, fullSize) {
    ...
    const s = ethereumjs_util_1.keccak(
      Buffer.concat([val, util_1.bufReverse(nonce)]),
      512
    );
    const mixhashes = Math.floor(
      util_1.params.MIX_BYTES / util_1.params.HASH_BYTES
    );
    ...
    for (i = 0; i < util_1.params.ACCESSES; i++) {
      const p =
        (util_1.fnv(i ^ s.readUInt32LE(0), mix.readUInt32LE((i % w) * 4)) %
  ...
  async _verifyPOW(header) {
    ...
    return (
      a.mix.equals(mixHash) &&
      ethereumjs_util_1.TWO_POW256.div(difficulty).cmp(result) === 1
    );
  }
