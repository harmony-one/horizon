...
class MmapDB {
  ...
  getMmapBuffer(name, size) {
    ...
    if (info.size < alignSize) fs.ftruncateSync(fd, alignSize);
...
class MerkelDB extends MmapDB {
  ...
  getElem(layer, index) {
    if (index >= layer.elemsNums)
      throw `exceed layers elemsNums: ${index} >= ${layer.elemsNums}`;
    const start = index * this.HASH_SIZE;
