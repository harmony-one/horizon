...
class MerkleTree {
    ...
    constructor (dirname, seed, cacheSize, fullSize, ethash) {
        ...
        if (!dagInfo) {
            ...
            for (let i = 0; i < LoopN; i++) {
                if (i % 128 == 0 && Date.now() - startTime > 10000) {
                    D(`leafLayer: ${i}/${LoopN}`)
                    startTime = Date.now()
                }
    ...
    getNextLayer (lowerLayer, layer) {
        ...
        for (let i = 0; i < layer.elemsNums; i++) {
            if (i % 128 == 0 && Date.now() - startTime > 10000) {
                D('NextLayer', i, layer.elemsNums)
                startTime = Date.now()
            }
            const index = 2 * i
            const elem1 = this.treedb.getElem(lowerLayer, index)
            const elem2 =
        index + 1 < lowerLayer.elemsNums
            ? this.treedb.getElem(lowerLayer, index + 1)
            : undefined
    ...
    getPairElement (idx, layer) {
        ...
        return pairIdx < layer.elemsNums
            ? this.treedb.getElem(layer, pairIdx)
            : undefined
    }
