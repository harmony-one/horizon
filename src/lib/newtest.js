...
getMmrProof(transactions.hash, '0xf').then((res) => {
    console.log(res)
})
...
async function deploy () {
    ...
    const epoch = response.result.epoch
    const mmrRoot = response.result.mmrRoot
    ...
    const mmrProof = await getMmrProof(transactions.hash, '0xd')

    console.log(mmrProof)

    const receiptProof = await txReceiptProof()
    ...
    try {
        await tokenLockerOnEthereum.validateAndExecuteProof(
            receiptProof.header,
            mmrProof.result,
            receiptProof,
