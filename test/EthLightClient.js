...
describe('Ethereum Light Client', function () {
    ...
    beforeEach(async function () {
        ...
        const rlpHeader = block1.header_rlp.substring(2)

        const bytes = hexToBytes(rlpHeader)
    ...
    it('Dummy Add Block TELC', async function () {
        const hashes = ['68776803263250831790672116253057717414537012912849465124179566887710318158907', '25', '12', '9999999', '1543254325', '2154326543656436543653']
        ...
        let CHead = (await ELC.canonicalHead()).toString()

        let parentHash = (await ELC.blocks(hashes[1])).parentHash.toString()
    ...
    describe('Forking tests', function () {
        it('Partial Fork and Total Difficulty Test', async function () {
            const prevHashes = ['68776803263250831790672116253057717414537012912849465124179566887710318158907', '2', '3', '4234', '555', '1', '2568', '9870', '990']
            const hashes = ['2', '3', '4234', '555', '1', '2568', '9870', '990', '437']
            const difficulties = ['400', '400', '400', '400', '400', '400', '400', '400', '400']
            const expectedTotalDifficulties = ['400', '800', '1200', '1600', '2000', '2400', '2800', '3200', '3600']
            const baseDifficulty = 5551247598

            const forkHashes = ['159', '435', '3456']
            const forkPrevHashes = ['1', '159', '435']
            const forkDifficulties = ['400', '1200', '1600']
            const expectedForkTotalDifficulties = ['400', '1600', '3200']
            const forkBaseDifficulty = 5551249598
            ...
            for (let i = 0; i < hashes.length; i++) {
                ...
                const totalDiff = (await ELC.blocks(hashes[i])).totalDifficulty.toString()
            ...
            for (let i = 0; i < forkHashes.length; i++) {
                ...
                // debug((await ELC.canonicalHead()).toString())
                const totalDiff = (await ELC.blocks(forkHashes[i])).totalDifficulty.toString()
                // debug(`Total Diff ${totalDiff}`);
                expect(parseInt(totalDiff) - forkBaseDifficulty).to.equal(parseInt(expectedForkTotalDifficulties[i]))
            }

            for (let i = 0; i < hashes.length; i++) {
                const isCanon = await ELC.canonicalBlocks(hashes[i])
            ...
            for (let i = 0; i < forkHashes.length; i++) {
                const isCanon = await ELC.canonicalBlocks(forkHashes[i])
        ...
        it('Complete fork replacement', async function () {
            ...
            const forkHashes = ['159', '435', '3456', '1114541', '5654681861', '16579165161', '156546813211', '159489515312', '259845', '25412541', '2548', '15547', '25814', '36525']
            const forkPrevHashes = ['125', '159', '435', '3456', '1114541', '5654681861', '16579165161', '156546813211', '159489515312', '259845', '25412541', '2548', '15547', '25814']
            const forkDifficulties = ['5551247598', '400', '400', '400', '400', '400', '400', '400', '400', '400', '400', '400', '400', '400']
            ...
            const isCanon = await ELC.canonicalBlocks(prevHashes[0])
